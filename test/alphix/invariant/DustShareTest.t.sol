...
contract DustShareTest is BaseAlphixTest {
    ...
    function test_dustSharesScenario() public {
        ...
        (uint256 amount0Large, uint256 amount1Large) =
            Alphix(address(hook)).previewAddReHypothecatedLiquidity(largeShares);
        ...
        console.log("  Amount0 in vault:", Alphix(address(hook)).getAmountInYieldSource(currency0));
        console.log("  Amount1 in vault:", Alphix(address(hook)).getAmountInYieldSource(currency1));
        ...
        (uint256 preview0, uint256 preview1) = Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(toWithdraw);
        ...
        console.log("  Total supply:", Alphix(address(hook)).totalSupply());
        console.log("  User1 balance:", Alphix(address(hook)).balanceOf(user1));
        console.log("  Amount0 in vault:", Alphix(address(hook)).getAmountInYieldSource(currency0));
        console.log("  Amount1 in vault:", Alphix(address(hook)).getAmountInYieldSource(currency1));

        // Now try to withdraw the last 1 share
        (uint256 preview0Last, uint256 preview1Last) = Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(1);
    ...
    function test_manySmallWithdrawals_leaveDust() public {
        ...
        console.log("  Total supply:", Alphix(address(hook)).totalSupply());
        console.log("  Amount0:", Alphix(address(hook)).getAmountInYieldSource(currency0));
        console.log("  Amount1:", Alphix(address(hook)).getAmountInYieldSource(currency1));

        // Withdraw in chunks, but leave 1 wei each time
        uint256 currentBalance = Alphix(address(hook)).balanceOf(user1);
        ...
        console.log("  Remaining shares:", Alphix(address(hook)).totalSupply());
        console.log("  Amount0 left:", Alphix(address(hook)).getAmountInYieldSource(currency0));
        console.log("  Amount1 left:", Alphix(address(hook)).getAmountInYieldSource(currency1));

        // Check what 1 share is worth
        (uint256 finalPreview0, uint256 finalPreview1) = Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(1);
    ...
    function test_dustFromLoss() public {
        ...
        // User2 deposits
        (uint256 amount0User2, uint256 amount1User2) = Alphix(address(hook)).previewAddReHypothecatedLiquidity(shares);
        vm.startPrank(user2);
        ...
        uint256 vault0Bal = Alphix(address(hook)).getAmountInYieldSource(currency0);
        uint256 vault1Bal = Alphix(address(hook)).getAmountInYieldSource(currency1);
        ...
        uint256 user1Shares = Alphix(address(hook)).balanceOf(user1);
        (uint256 preview0, uint256 preview1) = Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(user1Shares);
        ...
        uint256 user2Shares = Alphix(address(hook)).balanceOf(user2);
        (uint256 preview0User2, uint256 preview1User2) =
            Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(user2Shares);
    ...
    function test_extremeDust_partialWithdrawal() public {
        ...
        uint256 largeShares = 1000e18;
        (uint256 amount0, uint256 amount1) = Alphix(address(hook)).previewAddReHypothecatedLiquidity(largeShares);
        ...
        for (uint256 i = 0; i < 50; i++) {
            uint256 balance = Alphix(address(hook)).balanceOf(user1);
        ...
        console.log("  Total supply:", Alphix(address(hook)).totalSupply());
        console.log("  User1 shares:", Alphix(address(hook)).balanceOf(user1));
        console.log("  Amount0:", Alphix(address(hook)).getAmountInYieldSource(currency0));
        console.log("  Amount1:", Alphix(address(hook)).getAmountInYieldSource(currency1));

        uint256 finalShares = Alphix(address(hook)).balanceOf(user1);
        (uint256 finalPreview0, uint256 finalPreview1) =
            Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(finalShares);
        ...
        if (finalShares >= 1) {
            (uint256 oneShareWorth0, uint256 oneShareWorth1) =
                Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(1);
    ...
    function test_userGetsZeroFromWithdrawal() public {
        ...
        (uint256 amount0Tiny, uint256 amount1Tiny) = Alphix(address(hook)).previewAddReHypothecatedLiquidity(tinyShares);
        ...
        uint256 user2Balance = Alphix(address(hook)).balanceOf(user2);
        console.log("\nUser2 balance:", user2Balance);

        if (user2Balance > 0) {
            (uint256 preview0, uint256 preview1) =
                Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(user2Balance);
    ...
    function test_lossAndRounding_createsUnbackedShares() public {
        ...
        (uint256 oneShareWorth0, uint256 oneShareWorth1) = Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(1);
        console.log("\n1 share worth:");
        ...
        (uint256 fullShareWorth0, uint256 fullShareWorth1) =
            Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(1e18);
        ...
        (uint256 halfWorth0, uint256 halfWorth1) = Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(500e18);
        console.log("\n500e18 shares worth:");
        console.log("  amount0:", halfWorth0);
        console.log("  amount1:", halfWorth1);

        // What about exactly totalSupply (all shares)?
        (uint256 allWorth0, uint256 allWorth1) = Alphix(address(hook)).previewRemoveReHypothecatedLiquidity(shares);
